...
export default {
    ...
    setup() {
        ...
        const click_page = page => { // 展示对应页数的内容
            if (page === -2) {
                page = current_page - 1;
            } else if (page === -1) {
                page = current_page + 1;
        ...
        const pull_page = page => {
            current_page = page;
            $.ajax({
                url: "https://app2883.acapp.acwing.com.cn/api/record/getlist/",
                data: {
                    page,
                },
        ...
        const stringTo2D = map => { // 将传来的map字符串转换为二维数组
            let g = [];
